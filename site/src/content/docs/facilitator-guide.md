...
description: "Run-of-show, kickoff talking points, debrief structure, and contingency playbook."
---
...
| T+00:30 | **Module 3 · Slide Deck Maker** (60 min, debrief embedded in Step 9) — everyone | Each attendee solo; floater roams; lead facilitator runs embedded debrief at ~T+01:23 |
| T+01:30 | Done |
...
2. **The commercial pivot** — Walk the [Seller Playbook Overview](../seller-playbook-overview/) "60-Second Narrative." Don't go deep — they'll see it firsthand in Module 3.
...
## Deck-Type Picker (Module 3)

In Module 3 every attendee picks **one of four deck types** to build. Brief intro at T+00:30; encourage the room to pick fast (1–2 min) and start their session.
...
- **Live fallback**: at minute 25 (transition out of Module 1), attendees who didn't pre-select pick from the 4 types. Module 3's Step 1 is the picker.
- **Day-of swap**: discouraged after ~10 min into Module 3 — they lose too much iteration time.

---

## Debrief Structure (5–7 min, embedded in Module 3 Step 9)

There is no standalone debrief block under the new format — it lives at the end of Module 3 (Step 9). Aim for 5–7 minutes and move fast. Round-robin **by deck type**:
...
|---|---|---|
| **GitHub App alpha access denied for the room** | Medium | **Workshop-blocker for Module 1 and the live Module 3 path.** Fallback plan: facilitator briefly demos the App, then everyone uses the **No-App CLI fallback** documented in Module 3. Pre-flight: confirm 100% of attendees have alpha access 24h before. |
| GitHub App alpha access denied for one attendee | Medium | Pivot them to the **No-App CLI fallback** in Module 3. They can still complete a customer deck locally with the native Copilot CLI. |
...
| Attendee falls behind in Module 1 setup | High | Floater pairs with them; even if they're 10 min late to Module 3, the deck flow is forgiving |
| Module 1 runs over 25 min | High (likely on first run) | Hard-stop at T+00:30; anyone still mid-setup pairs with floater during Module 3 |

---

## Facilitator Pre-Flight (Week Before)

- [ ] Send pre-event form with the 4 Module 3 deck-type summaries; collect selections.
...
- [ ] Pre-run Module 1 + Module 3 (all 4 deck types) end-to-end yourself; record any "gotcha" notes.
